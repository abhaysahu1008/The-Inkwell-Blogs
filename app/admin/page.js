...
export default function AdminPage() {
  ...
  const fetchAll = async () => {
    const [postsRes, commentsRes, usersRes] = await Promise.all([
      ...
      supabase
        .from('comments')
        .select('id, comment_text, created_at, posts(title), users!user_id(name)')
        .order('created_at', { ascending: false }),
      supabase
        .from('users')
  ...
  const deleteComment = async (id) => {
    ...
    await fetch('/api/comments', {
      method: 'DELETE',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ comment_id: id, user_role: 'admin' }),
    })
  ...
  return (
    <>
      ...
      <main className="container" style={{ padding: '2rem 1.5rem 4rem' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: '1rem', marginBottom: '2rem' }}>
          {[
            ...
            <div key={stat.label} style={{ background: 'var(--white)', border: '1px solid var(--border)', borderRadius: '4px', padding: '1.25rem', textAlign: 'center' }}>
              <div style={{ fontSize: '2rem', fontFamily: "'Playfair Display', serif", fontWeight: 800, color: 'var(--accent)' }}>{stat.value}</div>
              <div style={{ fontSize: '0.78rem', color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: '0.05em', fontFamily: "'DM Mono', monospace", marginTop: '0.25rem' }}>{stat.label}</div>
            </div>
          ))}
        </div>
            ...
            <table className="admin-table">
              ...
              <tbody>
                {posts.map(post => (
                  <tr key={post.id}>
                    <td>
                      <Link href={`/blog/${post.id}`} style={{ color: 'var(--accent)', fontWeight: 500 }}>
                        {post.title}
                      </Link>
                    </td>
                    <td>{post.users?.name}</td>
                    <td style={{ fontFamily: "'DM Mono', monospace", fontSize: '0.82rem', color: 'var(--muted)' }}>
                ...
                {comments.map(c => (
                  <tr key={c.id}>
                    <td style={{ maxWidth: '300px' }}>
                      <span style={{ fontSize: '0.875rem' }}>{c.comment_text?.slice(0, 80)}{c.comment_text?.length > 80 ? '...' : ''}</span>
                    </td>
                    <td>{c.users?.name}</td>
                    <td style={{ color: 'var(--muted)', fontSize: '0.85rem' }}>{c.posts?.title?.slice(0, 30)}...</td>
                    <td style={{ fontFamily: "'DM Mono', monospace", fontSize: '0.82rem', color: 'var(--muted)' }}>
      ...
      <footer className="footer">
        <p>The <span>Inkwell</span> · Admin Panel</p>
      </footer>
    </>
  )
}
